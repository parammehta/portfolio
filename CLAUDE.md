...
- Auto-merge requires a **PAT** stored as the `RELEASE_PLEASE_TOKEN` secret (Contents + Pull requests read/write), because merges made with the default `GITHUB_TOKEN` do not re-trigger the workflow — so the release/tag/deploy would never fire. "Allow auto-merge" must also be enabled in repo settings.
